...
export default function Dashboard() {
  ...
  // Portão de assinatura — bloqueia acesso ao dashboard se não for assinante ativo ou em teste grátis (trial)
  if (subscriptionStatus !== null && subscriptionStatus !== 'active' && subscriptionStatus !== 'trial') {
    return <SubscriptionGate userEmail={session.user?.email || ''} subscriptionStatus={subscriptionStatus} />;
  }
    ...
    <div className="min-h-screen bg-[#06090e] text-[#f0f6fc] flex flex-col lg:flex-row font-sans">
      ...
      <aside className={`${showMobileMenu ? 'flex' : (isSidebarCollapsed ? 'hidden' : 'hidden lg:flex')} fixed lg:static inset-0 lg:inset-auto z-40 w-full lg:w-[270px] bg-[#080b10] border-r border-[#00ff9d]/10 flex-col shrink-0 h-screen transition-all duration-300 print:hidden`}>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {/* Card Modo de Teste Ativo */}
          {subscriptionStatus === 'trial' && (
            <div className="mx-3 my-4 p-4 rounded-xl bg-gradient-to-br from-[#10b981]/15 to-[#00ff9d]/5 border border-[#10b981]/30 space-y-3 print:hidden">
              <div className="flex items-center gap-2">
                <span className="flex h-2 w-2 relative">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#10b981] opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-[#10b981]"></span>
                </span>
                <span className="text-xs font-black text-white uppercase tracking-wider">Teste Ativo</span>
              </div>
              <div className="space-y-1.5">
                <div className="flex justify-between text-[11px]">
                  <span className="text-gray-400">Limite Clientes:</span>
                  <span className="font-semibold text-gray-200">1 / 1</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-gray-400">Limite Consultas:</span>
                  <span className="font-semibold text-gray-200">1 / 1</span>
                </div>
              </div>
              <button
                onClick={() => setShowPaywallModal(true)}
                className="w-full py-2 bg-[#10b981] hover:bg-[#05c475] text-gray-900 font-bold rounded-lg text-xs tracking-wider transition-all uppercase shadow-md shadow-[#10b981]/15 active:scale-[0.98]"
              >
                Virar Premium ⚡
              </button>
            </div>
      ...
      {/* Modal de Paywall Embutido */}
      {showPaywallModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#161b22] border border-gray-800 rounded-2xl w-full max-w-[460px] p-6 shadow-2xl space-y-5 relative">
            <button 
              onClick={() => setShowPaywallModal(false)}
              className="absolute top-4 right-4 text-gray-500 hover:text-gray-300 text-sm font-bold"
            >
              ✕
            </button>

            <div className="text-center space-y-1">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#10b981]/10 border border-[#10b981]/25 text-[10px] font-bold text-[#10b981] mb-2 uppercase tracking-wider">
                ⚡ Plano FocusLocal Premium
              </div>
              <h3 className="text-2xl font-black text-white">Ativação <span className="text-[#00ff9d]">Premium Imediata</span></h3>
              <p className="text-gray-400 text-xs leading-relaxed">
                Faça o upgrade agora para remover os limites de 1 cliente e de consultas de ranking!
              </p>
            </div>
            ...
            <div className="bg-[#0d1117] border border-gray-800 rounded-xl p-4 flex flex-col gap-3">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm font-bold text-white font-mono">42067991809</p>
                </div>
                <button
                  onClick={() => navigator.clipboard.writeText('42067991809')}
              ...
              <div className="flex flex-col gap-1 text-[11px] text-gray-400 bg-[#161b22] p-2.5 rounded-lg border border-gray-800">
                <p>💰 Valor: <strong>R$ 89 / mês</strong></p>
                <p>🏦 Banco: <strong>Santander</strong></p>
                <p>👤 Nome: <strong>Gabriel de Amorim Lima</strong></p>
              </div>
            </div>

            <div className="bg-[#10b981]/5 border border-[#10b981]/15 rounded-xl p-3 text-[11px] text-[#10b981]/90 flex items-start gap-2">
              <span className="text-base leading-none mt-0.5">🚀</span>
              <span>
                Para acelerar a liberação, envie o comprovante do PIX no WhatsApp abaixo. Seu acesso Premium será ativado na hora!
              </span>
            </div>

            <div className="flex flex-col gap-2">
              <a
                href={`https://wa.me/5511965843545?text=${encodeURIComponent(`Olá! Acabei de realizar o pagamento via PIX para assinar o FocusLocal Premium. Meu e-mail de cadastro é: ${session?.user?.email}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-black rounded-xl text-xs flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-950/20 active:scale-[0.98]"
              >
              ...
              <button 
                onClick={() => setShowPaywallModal(false)}
                className="w-full py-2.5 bg-transparent hover:bg-gray-800/30 text-xs font-bold text-gray-400 rounded-xl transition-all"
              >
                Voltar ao Dashboard
              </button>
            </div>
